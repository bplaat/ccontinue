...
def compile_code(code: str) -> str:
    ...
    def convert_class(match: re.Match[str]) -> str:
        ...
        for method_parts in re.findall(
            ...
            if name in class_.methods:
                ...
                class_.methods[name] = Method(name, return_type, is_virtual, arguments, class_.name)

        # ==== Generate missing methods ====
        g = ""
        if parent_class is not None:
            # Init method
            field_with_init_attribute = next(
                (field for field in class_.fields.values() if "init" in field.attributes), None
            )
            if class_.methods["init"].class_ != class_.name and field_with_init_attribute is not None:
                arguments = [
                    Argument(field.name, field.type) for field in class_.fields.values() if "init" in field.attributes
                ]
                class_.methods["init"] = Method("init", "void", False, arguments, class_.name)

                init_method = class_.methods["init"]
                parent_init_method = classes[parent_class.name].methods["init"]

                g += f"void _{to_snake_case(class_.name)}_init("
                g += ", ".join(
                    [f"{class_.name} *this"]
                    + [f"{argument.type} {argument.name}" for argument in init_method.arguments]
                )
                g += ") {\n"
                g += f"    {to_snake_case(parent_class.name)}_init("
            ...
            # Free method
            field_with_free_attribute = next(
                (field for field in class_.fields.values() if "free" in field.attributes), None
            )
            if class_.methods["free"].class_ != class_.name and field_with_free_attribute is not None:
                class_.methods["free"].class_ = class_.name

                g += f"void _{to_snake_case(class_.name)}_free({class_.name} *this) {{\n"
                for field in class_.fields.values():
                ...
                g += f"    _{to_snake_case(class_with_free.name)}_free(({class_with_free.name} *)this);\n"
                g += "}\n\n"
            ...
            for field in class_.fields.values():
                if field.class_ == class_.name and ("get" in field.attributes or "prop" in field.attributes):
                    ...
                    class_.methods[method_name] = Method(method_name, field.type, False, [], class_.name)

                    g += f"{field.type} _{to_snake_case(class_.name)}_get_{field.name}({class_.name} *this) {{\n"
            ...
            for field in class_.fields.values():
                if field.class_ == class_.name and ("set" in field.attributes or "prop" in field.attributes):
                    ...
                    class_.methods[method_name] = Method(
                        method_name, "void", False, [Argument(field.name, field.type)], class_.name
                    )

                    g += f"void _{to_snake_case(class_.name)}_set_{field.name}({class_.name} *this, "
            ...
            if not class_.is_abstract:
                init_method = class_.methods["init"]
                g += f"{class_.name} *{to_snake_case(class_.name)}_new("
                ...
                g += (
                    ...
                    + f"    {to_snake_case(class_.name)}_init("
        ...
        for method in class_.methods.values():
            if method.is_virtual:
                if method.class_ != current_class_name:
                    c += f"    // {method.class_}\n"
                    current_class_name = method.class_
        ...
        for method in class_.methods.values():
            if method.class_ == class_.name:
                c += (
                    f"{method.return_type} _{to_snake_case(class_.name)}_{method.name}("
                    + ", ".join(
        ...
        for method in class_.methods.values():
            ...
            c += f"#define {to_snake_case(class_.name)}_{method.name}("
    ...
    def convert_method(match: re.Match[str]) -> str:
        ...
        c = f"{return_type.strip()} _{to_snake_case(class_name)}_{method_name}({class_name} *this{arguments_str}) {{"
